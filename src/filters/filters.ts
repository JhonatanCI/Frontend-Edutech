...
export const getCategoriesByLearningResult = (
  learningResults: LearningResult[],
) => {
  return learningResults.map((lr) => lr.name);
};

export const getCategoriesForCourse = (
  contributions: SimpleLearningResult[],
) => {
  return contributions.map((lr) => lr.name);
};

export const getContributionForEachCategory = (
  programCourses: ProgramCourse[],
  categories: string[],
) => {
  return categories.map((category) =>
    programCourses.reduce((sum, course) => {
      const contributesToCategory = course.learningResultsContribution.some(
        (lr) => lr.name === category,
      );
...
export function filterCourses(courses: Course[], query: string): Course[] {
  ...
  keywords.forEach((keyword) => {
    courses.forEach((course) => {
      if (
        course.academicWorlds.some((world) =>
          world.toLowerCase().includes(keyword),
        )
      ) {
...
export const exactCoincidences = (pc: ProgramCourse, c: Course[]) => {
  const programCourseLearningResultsSet = new Set(
    pc.learningResultsContribution.map((lr) => lr.name),
  );

  return c.filter((course) => {
    if (
      course.learningResults.length !== programCourseLearningResultsSet.size
    ) {
      return false;
    }

    return course.learningResults.every((lr) =>
      programCourseLearningResultsSet.has(lr),
    );
  });
};
