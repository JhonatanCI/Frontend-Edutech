    ...
    () => {
      const fetchData = async () => {
        if (debouncedValue.trim() !== "") {
          setCurrentWord(debouncedValue);
          try {
            const results = await getResults(debouncedValue);
            console.log("🚀 ~ fetchData ~ results:", results)
            updateItems(results); // asume que `results` es un array de Course | Program | MicroLearning
          } catch (error) {
